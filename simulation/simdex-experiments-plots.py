...
ROOT_DIR = "./experiment_results_december_3"
...
def main():
    for variant in VARIANTS:
        ...
        for seed in seeds:
            ...
            fig, ax = plt.subplots(3, 3, sharex=False, sharey=False, figsize=(7.12, 3.50), gridspec_kw={'height_ratios': [1.5, 1.5, 1]})
            ...
            for idx_experiment, experiment in enumerate(EXPERIMENTS):
                ...
                # summed_df.plot(y="utility", ax=ax[0,idx_experiment], grid=True, label=EXPERIMENTS_LABELS[experiment], linestyle=EXPERIMENTS_LINESTYLES[experiment], color="black")
                ...
                df_expected_costs.plot(y="expected_cost", ax=ax[1,idx_experiment], grid=True,  linestyle=EXPERIMENTS_LINESTYLES[experiment], color="black", legend=False, ylim=(2, 7))
                ...
                newax = ax[2, idx_experiment]
                im = newax.imshow(strategies_image / 2.0, cmap=newcmp, aspect="auto", interpolation=None)
            ...
            #     ax[1,idx_experiment].axvline(x=val, ymax=0.05, linestyle="--", color="black", label="Coordination events")
                ...
                ax[2,idx_experiment].set_xlabel("Date (years)")
            # ax[0,idx_experiment].set_ylabel("Average cost")
            # ax[0,idx_experiment].set_xlabel("")
            ax[0,0].set_ylabel("Real cost")
            ax[1,0].set_ylabel("Expected cost")
            ax[2,0].set_ylabel("Active policy")
        ...
        for idx, row in totals_per_baseline.iterrows():
            # print("{} & {:.2f}\% & {:.2f}\% & {:.2f}\% & {:.2f} & {:.2f} & {:.2f} \\\\ \n \\hline".format(EXPERIMENTS_LABELS[idx], row["ontime"], row["delayed"], row["late"], row["workers"], row["utility"], row["expected_cost"]))
            print("{} & {:.2f}\% & {:.2f}\% & {:.2f} & {:.2f} & {:.2f} \\\\ \n \\hline".format(EXPERIMENTS_LABELS[idx], row["delayed"], row["late"], row["workers"], row["utility"], row["expected_cost"]))
